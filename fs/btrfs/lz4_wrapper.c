			      ...
			      int hi)
{
	...
	*total_in = 0;
	...
	while (tot_in < len) {
		if (hi)
			out_len = LZ4_compress_HC(data_in, workspace->cbuf, in_len, LZ4_MAX_WORKBUF, 9, workspace->mem);
		else
			out_len = LZ4_compress_default(data_in, workspace->cbuf, in_len, LZ4_MAX_WORKBUF, workspace->mem);
		if (out_len == 0) {
			printk(KERN_DEBUG
...
cont:
		...
		out_len = LZ4_decompress_safe(buf, workspace->buf, in_len, LZ4_CHUNK_SIZE);
			  ...
			  size_t srclen, size_t destlen)
{
	...
	out_len = LZ4_decompress_safe(data_in, workspace->buf, in_len, LZ4_CHUNK_SIZE);
